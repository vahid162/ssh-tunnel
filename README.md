...
## آیا این ریپو برای اجرا روی سرور ایران «آماده» است؟

پاسخ کوتاه: **برای سناریوی عملی ایران → خارج آماده است** و برای چند تانل هم طراحی شده، اما مثل هر اسکریپت عملیاتی باید با چک‌لیست زیر اجرا شود.

### چک‌لیست آمادگی قبل از Production

1. هر دو سرور Linux باشند و `root` یا دسترسی sudo واقعی داشته باشی.
2. روی خارج، SSH در دسترس باشد و `PermitTunnel yes` فعال شود (اسکریپت انجام می‌دهد).
3. روی ایران، `TUN ID` یکتا انتخاب کنی (برای هر تانل یک ID جدا).
4. پورت‌های DNAT هر تانل با تانل/سرویس دیگر overlap نداشته باشند.
5. بعد از نصب، خروجی `systemctl status` و `ip a show tunX` را چک کنی.

### پشتیبانی چند تانل به چند سرور خارج

بله؛ می‌توانی چند پروفایل جدا بسازی. برای هر مقصد خارج:

- یک `TUN ID` جدید بده (مثلاً 5، 6، 7)
- یک subnet تونل جدا بده (مثلاً `192.168.83.0/30`، بعدی `192.168.84.0/30`)
- پورت‌های ورودی هر پروفایل را جدا نگه دار

اسکریپت برای هر پروفایل یک سرویس جدا می‌سازد (`ssh-tun<TUN_ID>-dnat.service`) و مدیریت پروفایل هم دارد.

### نکات استانداردسازی (شفاف و مهم)

این پروژه برای setup سریع بسیار خوب است، ولی اگر «استاندارد سازمانی سخت‌گیرانه» بخواهی، معمولاً این موارد را هم اضافه می‌کنند:

- مانیتورینگ مرکزی (Prometheus/Grafana + alert)
- جمع‌آوری لاگ مرکزی
- بکاپ/HA برای سناریوهای حیاتی
- سیاست امنیتی دقیق‌تر برای SSH (user محدود، key rotation، fail2ban)

پس نتیجه عملی: برای استفاده واقعی مناسب است، اما برای سطح Enterprise باید روی عملیات و مانیتورینگ کامل‌ترش کنی.
